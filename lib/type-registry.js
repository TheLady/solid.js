...
function initTypeRegistry (profile, containerUrl, options) {
  ...
  return webClient.post(containerUrl, publicIndex.data, publicIndex.slug, options)
    .then(function (metaPublic) {
      // patch profile
      var newTriple = rdf.st(
        rdf.sym(profile.webId),
        vocab.solid('publicTypeIndex'),
        rdf.sym(metaPublic.url)
      ).toNT()

      return webClient.patch(profile.webId, [], [newTriple], options)
        .then(function (result) {
          profile.typeIndexListed.uri = util.absoluteUrl(containerUrl,
                                                          metaPublic.url)
          console.log(profile)
          ...
          return webClient.post(containerUrl, privateIndex.data,
            ...
            .then(function (metaPrivate) {
              profile.typeIndexUnlisted.uri = util.absoluteUrl(containerUrl,
                                                                metaPrivate.url)
              // patch profile
              var newTriple = rdf.st(
                rdf.sym(profile.webId),
                vocab.solid('privateTypeIndex'),
                rdf.sym(metaPrivate.url)
              ).toNT()

              return webClient.patch(profile.preferences.uri, [], [newTriple],
                                    options)
                .then(function (result) {
                  return profile.loadTypeRegistry(options)
                })
